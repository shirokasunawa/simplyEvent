...
function template_message_gauche(content, date) {
    ...
    s += '<i class="bi bi-cart-plus"></i>'
    s += '<p>' + content + '</p>'
    s += '<span class="time-right">' + date + '</span>'
    s += '</div>'
    return s
}
...
function initMessage() {
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            res = xhr.response;
            console.log(res)
            if (res[0]["_conversation"].length != 0) {
                var s = ''
                for (var f = 0; f < res[0]["_conversation"].length; f++) {

                    s += '  <div class="btn " style="width: 100%;margin-top:1em;" onclick="afficheMessage(\'' + res[0]["_conversation"][f]["_id"] + '\')"> <div class=" d-inline p-2 justify-content-start"> ' + res[0]["_conversation"][f]["_idUserSociety"]["nameSociety"] + ' </div><div class="d-inline p-2 justify-content-end">' + res[0]["_conversation"][f]["_product"]["typeProduct"] + ' : ' + res[0]["_conversation"][f]["_product"]["priceProduct"] + ' €</div></div>'
                }
